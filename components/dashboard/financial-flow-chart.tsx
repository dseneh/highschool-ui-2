...
function getDataForPeriod(period: TimePeriod, data: ChartPoint[]) {
  switch (period) {
    case "3months":
      return data.slice(-3);
    case "6months":
...
export function FinancialFlowChart({ data = fallbackData }: { data?: ChartPoint[] }) {
  ...
  const chartData = getDataForPeriod(period, data);
