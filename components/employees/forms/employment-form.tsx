...
const employmentSchema = z.object({
  ...
  employment_status: z.enum(["active", "inactive", "suspended", "terminated", "on_leave", "retired"]),
...
export function EmploymentForm({
  ...
  const { data: staffListData } = employeeApi.getEmployees({ page_size: 1000 })

  const staffList = useMemo<EmployeeDto[]>(() => {
    if (!staffListData) return []
    if (Array.isArray(staffListData)) return staffListData as EmployeeDto[]
    if (Array.isArray((staffListData as { results?: EmployeeDto[] }).results)) return (staffListData as { results: EmployeeDto[] }).results
    return []
  }, [staffListData])

  // Filter out current staff and those with circular dependencies
  const availableManagers = useMemo(() => {
    return staffList.filter(s => s.id !== staff.id && s.employment_status === 'active')
  }, [staffList, staff.id])

  const positionId = typeof staff.position === "object" ? staff.position?.id : undefined
  const departmentId = typeof staff.department === "object" ? staff.department?.id : undefined
  ...
  const {
    ...
    defaultValues: {
      ...
      employment_status: (staff.employment_status as EmploymentFields["employment_status"]) || "active",
      position: positionId || NONE_VALUE,
      department: departmentId || NONE_VALUE,
    ...
    <form
      onSubmit={handleSubmit((values) => {
        onSubmit({
          ...
          department: values.department === NONE_VALUE ? null : values.department,
