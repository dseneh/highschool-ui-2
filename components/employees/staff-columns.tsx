...
import { EmployeeListItem } from "@/lib/api2/employee/types";
...
interface StaffColumnsProps {
  ...
  onDelete?: (staff: EmployeeListItem) => void;
  onEdit?: (staff: EmployeeListItem) => void;
  onStatusAction?: (staff: EmployeeListItem, action: StaffStatusActionType) => void;
  returnToUrl?: string;
  basePath?: string;
  entityLabel?: string;
...
}: StaffColumnsProps = {}): ColumnDef<EmployeeListItem>[] {
...
function StaffActionsCell({
  ...
  const statusActions: Array<{
  ...
  }|any> = [
    {
      ...
      icon: Ban,
      action: "suspend",
      visible: staff.employment_status !== "suspended" && staff.employment_status !== "terminated",
    },
    {
      label: `Terminate ${entityLabel}`,
      icon: UserX,
      action: "terminate",
      visible: staff.employment_status !== "terminated",
      destructive: true,
    },
    {
      label: "Mark On Leave",
      ...
      visible: staff.employment_status !== "on_leave" && staff.employment_status !== "terminated",
    },
    {
      label: "Restore Active",
      icon: RotateCcw,
      action: "activate",
      visible: staff.employment_status !== "active",
    },
