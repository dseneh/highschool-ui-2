...
function getStaffList(data: unknown): EmployeeListItem[] {
  if (Array.isArray(data)) return data as EmployeeListItem[];
...
export function StaffSelectDialog({
  ...
  const rawApi = useEmployeeApi();
  const queryClient = getQueryClient();

  const { data, isLoading } = employeeApi.getEmployees(
    { page_size: 1000 },
    { enabled: open }
  );
  ...
  const toggleTeacher = useMutation({
    mutationFn: async ({ id, next }: { id: string; next: boolean }) => {
      const response = await rawApi.patchEmployeeApi(id, { is_teacher: next });
  ...
  const handleToggle = async (staff: EmployeeListItem) => {
