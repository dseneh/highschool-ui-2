...
import type { EmployeeListItem, EmployeeListResponse } from "@/lib/api2/employee/types";
...
export function StaffTable({
  ...
  const [tableInstance, setTableInstance] = React.useState<Table<EmployeeListItem> | null>(null);
  const isApplyingUrlFilters = React.useRef(false);
  const previousColumnFilters = React.useRef<string>("");

  const [deleteStaff, setDeleteStaff] = React.useState<EmployeeListItem | null>(null);
  const [deleteDialogOpen, setDeleteDialogOpen] = React.useState(false);
  const [deleteConfirmed, setDeleteConfirmed] = React.useState(false);

  const [statusAction, setStatusAction] = React.useState<{
    type: StaffStatusActionType;
    staff: EmployeeListItem;
  } | null>(null);
  ...
  const { mutateAsync: performDelete, isPending: isDeleting } =
    employeeApi.deleteEmployee(deleteStaff?.id || "");

  const { mutateAsync: patchStaffStatus, isPending: isUpdatingStatus } =
    employeeApi.patchEmployee(statusAction?.staff.id || "");

  const handleDelete = React.useCallback((staff: EmployeeListItem) => {
    setDeleteStaff(staff);
    setDeleteConfirmed(false);
    setDeleteDialogOpen(true);
  }, []);

  const handleStatusAction = React.useCallback((staff: EmployeeListItem, action: StaffStatusActionType) => {
    ...
    switch (action) {
      case "suspend":
        return {
          ...
          getSuccessMessage: (staff: EmployeeListItem) => `${staff.full_name} has been suspended.`,
          requiresReason: true,
          requiresDate: true,
      ...
      case "terminate":
        return {
          ...
          getSuccessMessage: (staff: EmployeeListItem) => `${staff.full_name} has been marked as terminated.`,
          requiresReason: true,
          requiresDate: true,
      ...
      case "mark_on_leave":
        return {
          ...
          getSuccessMessage: (staff: EmployeeListItem) => `${staff.full_name} is now marked as on leave.`,
      ...
      default:
        return {
          ...
          getSuccessMessage: (staff: EmployeeListItem) => `${staff.full_name} is now active.`,
          requiresReason: false,
          requiresDate: false,
  ...
  const staffRows = React.useMemo<EmployeeListItem[]>(() => data?.results || [], [data]);
  ...
  React.useEffect(() => {
    ...
    applyArrayFilter("employment_status", urlParams.status);
  ...
  React.useEffect(() => {
    ...
    const handleStateChange = () => {
      ...
      columnFilters.forEach((filter) => {
        if (filter.id === "employment_status") {
  ...
  const handleRowClick = React.useCallback(
    (staff: EmployeeListItem) => {
      ...
      router.push(`/employees/${staff.id_number}?returnTo=${encodeURIComponent(returnTo)}`);
    },
    [router]
  );
