...
import type { Department, Position } from "@/lib/api2/staff/types";
...
export function StaffFormModal({
  ...
  //   if (Array.isArray(positionsData)) return positionsData as Position[];
  //   if (Array.isArray(positionsData.results)) return positionsData.results as Position[];
  //   return [];
  // }, [positionsData]);


  // const departmentOptions = React.useMemo<Department[]>(() => {
  //   if (!departmentsData) return [];
  //   if (Array.isArray(departmentsData)) return departmentsData as Department[];
  //   if (Array.isArray(departmentsData.results)) return departmentsData.results as Department[];
  //   return [];
  // }, [departmentsData]);

  const positionLabelMap = React.useMemo(() => {
    if (!positionsData) return new Map();
    return new Map(
  ...
  const departmentLabelMap = React.useMemo(() => {
    if (!departmentsData) return new Map();
    return new Map(
      departmentsData.map((department: any) => [department.id, department.name])
    );
  }, [departmentsData]);
  ...
  async function handleSubmit() {
    const payload: StaffFormSchema = {
      ...
      primary_department: form.primary_department || undefined,
